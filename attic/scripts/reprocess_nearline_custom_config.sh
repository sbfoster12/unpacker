...
DETECTOR_MAPPING_FILE=$1
RUN1=$2
RUN2=$3
echo "Processing all files with ${DETECTOR_MAPPING_FILE} from $RUN1 to $RUN2"
...
do
    ...
    if [ ${RUN} -ge ${RUN1} ] && [ $RUN -le ${RUN2} ]; then
        # echo "More than allowed number of nearline instances. Skip this file."
        echo "Found fun to process ${RUN} | ${SUBRUN} | ${FILE}"
    else
