...
MIDAS_OUTPUT_DIR="/data1/online"
NEARLINE_OUTPUT_DIR="/home/pioneer/pioneer/pioneer-teststand/nearline"
LOG_OUTPUT_DIR="/home/pioneer/pioneer/pioneer-teststand/nearline_logs"
DETECTOR_MAPPING_FILE="detector_mapping.json"
VERBOSITY=0
PROCESS_DIR="/home/pioneer/pioneer/pioneer-teststand/packages/unpacking/bin"

cd $PROCESS_DIR
echo "Running from:" $PROCESS_DIR

file_list_new=`find "${MIDAS_OUTPUT_DIR}" -type f -mmin +1 -mmin -30 -name '*.mid.lz4' | shuf`
file_list_old=`find "${MIDAS_OUTPUT_DIR}" -type f -mmin +30 -name '*.mid.lz4' | shuf`
...
do
    ...
    if [ ${RUN} -lt 508 ]; then
        # echo "More than allowed number of nearline instances. Skip this file."
        echo "Skipping run ${RUN} | ${SUBRUN} | ${FILE}"
        continue
    fi
    ...
    # TODO: update a database of processed runs
    echo "pioneer_unpacker $i $VERBOSITY $DETECTOR_MAPPING_FILE $ROOT_FILE" 
    time ./pioneer_unpacker $i $VERBOSITY $DETECTOR_MAPPING_FILE $ROOT_FILE > $LOG_FILE 2>&1
    NEARLINE_EXIT=$?
    ...
    #     '$i', '${ROOT_FILE}', '${LOG_FILE}', ${NEARLINE_EXIT} );"
    # echo $psql_string >> ${LOG_FILE}
    # echo $psql_string 
    # /home/pioneer/pioneer/pioneer-teststand/packages/psql/postgresql-16.1/install/bin/psql -U pioneer_writer -d pioneer_online -c "$psql_string" >> ${LOG_FILE} 
